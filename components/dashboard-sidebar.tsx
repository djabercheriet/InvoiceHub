...
function SidebarNavItem({ item, pathname, isCollapsed }: SidebarNavItemProps) {
  ...
  if (!hasSubItems) {
    return (
      ...
      >
        <item.icon className={cn("w-5 h-5 shrink-0", pathname === item.href ? "text-primary" : "group-hover/item:text-primary")} />
  ...
  return (
    <div className="space-y-1">
      <button
        ...
        className={cn(
          "w-full flex items-center justify-between px-3 py-2 rounded-lg transition-all duration-200 group/item",
          isActive ? "text-foreground font-bold" : "text-muted-foreground hover:bg-secondary/30 hover:text-foreground"
        )}
      >
        ...
        {!isCollapsed && (
          <ChevronDown className={cn("w-4 h-4 transition-transform duration-200 opacity-40", isOpen ? "rotate-0" : "-rotate-90")} />
        )}
      </button>
          ...
          {item.items?.map((sub: any) => {
            ...
            return (
              <Link
                ...
                href={sub.href}
                className={cn(
                  "flex items-center gap-2 py-1.5 px-2 rounded-md text-xs font-medium transition-colors",
                  subActive 
...
export function DashboardSidebar({ isSuperAdmin, signOutAction }: SidebarProps) {
 ...
 const navItems = [
    ...
    { 
      ...
      icon: ShoppingBag, 
      items: [
        { href: '/dashboard/sales/quotes', label: 'Quotes & Proposals' },
        { href: '/dashboard/sales/invoices', label: t('invoices') },
        { href: '/dashboard/sales/customers', label: t('customers') },
      ]
    },
    { 
      label: 'Inventory', 
      icon: Package, 
      items: [
        { href: '/dashboard/inventory', label: 'Products Master' },
    ...
    { 
      ...
      icon: BarChart3, 
      items: [
        { href: '/dashboard/finance', label: 'Overview & P&L' },
    ...
    { 
      ...
      icon: Zap, 
      items: [
        { href: '/dashboard/automation', label: 'Workflows & Automation' },
      ]
    },
    { 
      label: 'Reports', 
      ...
      items: [
        ...
        { href: '/dashboard/reports/inventory', label: 'Inventory Reports' },
      ]
    },
 ...
 <aside 
 className={cn(
"relative h-screen bg-card border-r border-border/50 flex flex-col sidebar-transition group/sidebar",
 ...
 <div className={cn("p-6 flex items-center gap-3", isCollapsed ?"justify-center" :"justify-start")}>
  <div className="w-10 h-10 rounded-xl overflow-hidden shadow-lg shadow-indigo-500/20 shrink-0">
 ...
 <p className="text-[10px] text-muted-foreground font-semibold tracking-wider mt-1">Dynamics</p>
   ...
   <nav className="flex-1 px-3 space-y-4 py-4 overflow-y-auto overflow-x-hidden">
