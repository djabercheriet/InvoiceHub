...
import { 
  FileText, ArrowLeft, Printer, Download, Mail, CheckCircle, Edit2,
  Clock, AlertCircle, ShoppingBag, ShoppingCart, User, Building,
  Calendar, CreditCard, Info, Loader2, History, Activity as ActivityIcon
} from "lucide-react";
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 px-4 lg:px-0">
      ...
      <div className="flex items-center justify-between border-b border-border/60 pb-6 top-header-actions">
        <Button variant="ghost" onClick={() => router.push("/dashboard/sales/invoices")} className="gap-2 font-bold tracking-tight text-muted-foreground hover:text-foreground">
        ...
        <div className="flex items-center gap-3">
          ...
          {isClient && invoice && isSale && (
            <PDFDownloadLink
              document={<InvoicePDF invoice={invoice} customer={customer} items={items} company={company} />}
              fileName={`MANIFEST-${invoice.invoice_number}.pdf`}
            >
              {/* @ts-ignore */}
              {({ loading: pdfLoading }) => (
                <Button variant="outline" size="sm" disabled={pdfLoading} className="gap-2 font-bold w-full md:w-auto h-9">
                  {pdfLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}
                  {pdfLoading ? "Compiling PDF..." : "Export"}
                </Button>
              )}
            </PDFDownloadLink>
          )}
          ...
          {isSale && isDraft && (
            <Button variant="outline" size="sm" onClick={() => router.push(`/dashboard/sales/invoices/${invoice.id}/edit`)} className="gap-2 font-bold w-full md:w-auto h-9 border-primary/20 text-primary hover:bg-primary/10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          ...
          <Card className="border-none shadow-xl glass-card manifest-status-card">
            <CardHeader className="pb-2">
              <CardTitle className="text-[10px] font-bold tracking-widest text-primary flex items-center gap-2">
          ...
          <Card className="border-none shadow-xl glass-card system-log-card">
            <CardHeader className="pb-2 border-b border-border/40 mb-4">
              <CardTitle className="text-[10px] font-bold tracking-widest text-muted-foreground flex items-center gap-2">
                <ActivityIcon className="w-3 h-3" /> Activity Timeline
              </CardTitle>
            </CardHeader>
            <CardContent>
              <ActivityTimeline entityId={invoice.id} entityType="invoice" />
            </CardContent>
          </Card>
        </div>
      </div>
